...
import { BookOpen, CheckCircle2, Clock3, Goal, GraduationCap, PauseCircle, Plus } from 'lucide-react-native';
...
import LearningBookModal, { type LearningBookDraft } from '@/components/learning/LearningBookModal';
...
function BookRow({ book, divided, onPress }: { book: Book; divided?: boolean; onPress: () => void }) {
  ...
  return (
    <TouchableOpacity
      activeOpacity={0.84}
      onPress={onPress}
      style={[styles.bookRow, divided && { borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: colors.border }]}
    >
      {book.coverUrl ? (
...
export default function LearningPrimaryV2() {
  ...
  const orderedBooks = useMemo(() => [...reading, ...wantToRead, ...pausedBooks, ...completedBooks], [reading, wantToRead, pausedBooks, completedBooks]);
  ...
  const saveBook = (draft: LearningBookDraft) => {
    const nowIso = new Date().toISOString();
    const currentBooks = profile?.favoriteBooks ?? [];
    if (editingBook) {
      updateProfile({
        favoriteBooks: currentBooks.map((book) => book.id === editingBook.id
          ? {
              ...book,
              ...draft,
              completedAt: draft.status === 'Completed' ? (book.completedAt ?? nowIso) : undefined,
              startedAt: draft.status === 'Reading' ? (book.startedAt ?? nowIso) : book.startedAt,
              updatedAt: nowIso,
            }
          : book),
      });
      ...
      const book: Book = {
        ...
        ...draft,
        startedAt: draft.status === 'Reading' ? nowIso : undefined,
        completedAt: draft.status === 'Completed' ? nowIso : undefined,
        createdAt: nowIso,
        updatedAt: nowIso,
      };
      updateProfile({ favoriteBooks: [book, ...currentBooks] });
    }
  ...
  const deleteBook = () => {
    ...
    updateProfile({ favoriteBooks: books.filter((book) => book.id !== editingBook.id) });
    setBookModalOpen(false);
    setEditingBook(null);
  };
    ...
    <>
      ...
      >
        ...
        <View style={styles.section}>
          <SectionHeader title="Your books" subtitle="What you are reading, saving and finishing." actionLabel="Add" onAction={openNewBook} />
          {books.length > 0 ? (
            <SurfaceCard variant="list">
              {orderedBooks.slice(0, 8).map((book, index) => (
                <BookRow key={book.id} book={book} divided={index > 0} onPress={() => openBook(book)} />
              ))}
            </SurfaceCard>
          ) : (
            <SurfaceCard>
              ...
              <View style={styles.emptyAction}>
                <ActionButton label="Add a book" kind="secondary" onPress={openNewBook} icon={<Plus size={15} color={colors.text} />} />
              </View>
            </SurfaceCard>
          )}
        </View>
      ...
      <LearningBookModal
        ...
        onSave={saveBook}
        onDelete={editingBook ? deleteBook : undefined}
      />
    </>
  );
}
